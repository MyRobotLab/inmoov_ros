...
void TeensyServo::readEeprom(int s) {
  // s is eeprom slot to read
  // luckily the digital pins are numbered 0 - 11, so we can just use those

  int offset = EEPROM_START + (s * EEPROM_RECORD_SIZE);
  EEPROM.get(offset, e);

  int newCheck = generateEepromChecksum();
  if (newCheck != e.checksum)
  {
    e.minPulse = MINPULSE;
    e.maxPulse = MAXPULSE;
    e.minAngle = MINANGLE;
...
void TeensyServo::writeEeprom() {
  int offset = EEPROM_START + (servoPin * EEPROM_RECORD_SIZE);
  e.checksum = generateEepromChecksum();
  EEPROM.put(offset, e);
}
